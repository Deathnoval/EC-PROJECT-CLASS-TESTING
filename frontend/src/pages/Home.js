...
const Home = () => {
  ...
  const categories = [
    { name: "Tiểu Thuyết", thumbnail: book5img },
    { name: "Văn Học", thumbnail: book6img },
    { name: "Thiếu Nhi", thumbnail: book7img },
    { name: "Kinh Tế", thumbnail: book8img },
    { name: "Ngôn Tình", thumbnail: book9img },
    { name: "Tâm Lí", thumbnail: book10img },
    { name: "Manga", thumbnail: book11img },
  ];
              ...
              <div className="categories d-flex justify-content-between align-items-center">
                {categories.map((item, index) => (
                  <div className="item" key={index}>
                    <h6 style={{ paddingLeft: "15px" }}>{item.name}</h6>
                    <div className="thumbnail">
                      <img
                        onClick={() => setCategorys(item.name)}
                        src={item.thumbnail}
                        alt={item.name} // Thêm alt để cải thiện accessibility (A11Y)
                      />
                    </div>
                  </div>
                ))}
